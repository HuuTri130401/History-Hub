jQuery(document).ready(function ($) {
    ...
    $(document).ready(function () {
        // Xử lý sự kiện khi nhấp vào các mục Timeline
        $(".timeline-link").on("click", function () {
            const timelineId = $(this).data("id"); // Lấy giá trị TimelineId từ thuộc tính data-id
            getTimelineById(timelineId); // Gọi hàm AJAX
        });
    });

    function getTimelineById(id) {
        // Gọi yêu cầu AJAX đến phương thức OnGetTimelineByIdAsync với giá trị TimelineId
        $.ajax({
            url: `/GuestAccess/HomeGuest?handler=TimelineById&id=${id}`, // URL đúng chưa?
            type: "GET",
            success: function (data) {
                // Xử lý dữ liệu khi nhận được từ yêu cầu AJAX thành công
                displayTimelineData(data.title, data.description);
            },
            error: function () {
                // Xử lý khi xảy ra lỗi trong yêu cầu AJAX
                console.log("Error occurred while fetching Timeline data.");
            }
        });
    }

    function displayTimelineData(title, description) {
        // Hiển thị dữ liệu Timeline lên trang web
        const timelineTitle = $("#timelineTitle");
        const timelineDescription = $("#timelineDescription");

        timelineTitle.text(title);
        timelineDescription.text(description);
    }
});
